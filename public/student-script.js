...
async function submitFeedback() {
    ...
    try {
        ...
        if (result.success) {
            ...
            // Optional: Reload feedback history if you implement it for students
...
// ==========================================================
// --- NEW: LOST & FOUND FUNCTIONS (student-reports-view) ---
// ==========================================================
...
async function submitLostItemReport() {
    ...
    try {
        ...
        if (result.success) {
            alert(result.message);
            form.reset();
            // Refresh the display of found items after submission
            fetchFoundItems(); 
        } else {
...
async function fetchFoundItems() {
    ...
    if (!lostFoundBody) return; // Guard clause

    lostFoundBody.innerHTML = `<tr><td colspan="4" class="text-center py-4 text-secondary-gray">Loading recently found items...</td></tr>`;
    
    try {
        // This endpoint fetches items that have been FOUND (for the student to check)
        const response = await fetch('/api/lost-found/found-items');
        const result = await response.json();

        if (result.success) {
            const items = result.foundItems;
            lostFoundBody.innerHTML = ''; 

            if (items.length === 0) {
                lostFoundBody.innerHTML = `<tr><td colspan="4" class="text-center py-4 text-accent-green font-semibold">🎉 No items currently reported as found by staff.</td></tr>`;
                return;
            }
            ...
            items.forEach(item => {
                ...
                const statusColor = item.status === 'Pending' ? 'text-info-yellow' : 'text-accent-green';
                ...
                lostFoundBody.innerHTML += `
                    <tr class="hover:bg-light-bg transition-colors">
                        ...
                        <td class="py-3 px-6 text-sm font-semibold ${statusColor}">${statusEmoji} ${item.status}</td>
                    </tr>
                `;
            });
        } else {
            lostFoundBody.innerHTML = `<tr><td colspan="4" class="text-center py-4 text-accent-red">Failed to load found items.</td></tr>`;
        }
    } catch (error) {
        console.error('Error fetching found items:', error);
        lostFoundBody.innerHTML = `<tr><td colspan="4" class="text-center py-4 text-accent-red">Network error loading found items.</td></tr>`;
    }
}
